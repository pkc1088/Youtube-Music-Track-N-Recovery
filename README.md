...
- 타 음악 플랫폼(Spotify, Apple Music 등)
  - 정식 음원이 아닌 라이브, 콘서트 영상 등을 포함하지 못하며 비교적으로 적은 수의 음악만 등록되어 있습니다.
...
<a id="-4-internal-policy-quota-optimization"></a>
...
- 케이스 별 API 사용량 공식(예시)
  ...
  - 비정상 영상 1개 복구<br>
    ...
    | 복구   | 대체 영상 검색<br>대체 영상 추가<br>비정상 영상 삭제 | **100 + 1 + 50 + 50**     | 201 |
<br>
...
1. **Redis를 활용한 유연한 동시성 제어 할당량 시스템 구축**
    - 문제:
      - YouTube API 일일 할당량의 다중 인스턴스 공유 및 사용자별 정확한 추적 필요
    
    - 해결:
      - 사용자별 일일 사용량 키로 독립적 추적 (Youtube API 재지급 기준에 따라 TTL 설정)
    
      - 동적 전역 할당량 키로 애플리케이션 재배포 없이 실시간 기본 할당량 조절 가능

2. **Lua Script를 이용한 원자적 연산과 Race Condition 해결**
    - 문제:
      - 클라이언트의 '읽기 - 쓰기' 로직과 백엔드 복구 프로세스의 동시 할당량 소비 요청 시 Lost Update 문제 발생
    
    - 해결:
      - 할당량 확인, 비교, 차감, TTL 설정 로직 전체를 Lua Script로 작성.

      - 단일 연산으로 Race Condition 해결

3. **페이지네이션 API 호출의 정확한 비용 측정**
    - 문제:
      - 페이지네이션을 사용하는 API는 최종적으로 몇 번의 호출이 발생할지 미리 알 수 없어 정확한 비용 측정이 어려움
    
    - 해결:
      - 한 페이지를 처리할 때마다 실시간으로 할당량을 확인하고 차감
    
      - 할당량 초과 시 즉시 작업 중단 후 `QuotaExceededException` 발생

4. **트랜잭션 롤백을 지원하는 '예약-반환' 모델과 Outbox 패턴 연동**
    - 문제:
      - Outbox에 작업을 명세하는 것과 실제 API 호출 사이 시간적 차이가 발생

      - Outbox 등록 후 추가 작업 진행 시 `QuotaExceededException` 발생해 롤백이 되면, 실제 소비하지 않은 할당량을 고객이 부담 
    
    - 해결:
      - **예약**: 서비스 트랜잭션 내에서, Outbox에 할당량 소모가 큰 작업(Add/Delete)을 기록하기 직전에 필요한 할당량을 **먼저 Redis에서 예약(소모)**
    
      - **롤백 및 반환**: 트랜잭션 내에서 `QuotaExceededException`가 발생하여 재생목록 단위의 롤백이 진행되기 직전, 누적된 예약량을 다시 반환하여 사용량 총계를 일관성 있게 유지

5. **In-Memory DB 활용**
    - 모든 할당량 관련 로직을 DB가 아닌 Redis에서 처리하여, DB I/O 없이 빠른 응답 속도를 확보하고 DB 부하를 줄임
<br><br>
...
- `RefreshToken` 통해 고객 부재에도 `AccessToken`을 발급해 복구의 자동화 실현
